...
const PHYSICIAN_NAV = [
  {
    href:        '/my-queue',
    title:       'My Queue',
    description: 'Referrals routed directly to you for review',
    icon:        'person',
...
function countByTier(referrals: ReferralSummary[], myUserId?: string) {
  return {
...
export default function HomePage() {
  ...
  const counts = countByTier(referrals, user.id)
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        ...
        {isPhysician ? (
          /* Physician home — My Queue + All Cases */
          <>
            <div className="mb-5">
              <h2 className="text-lg font-bold tracking-tight text-primary">Your Dashboard</h2>
              <p className="text-sm text-on-surface-variant mt-1">Referrals assigned to you and the full clinic record.</p>
            </div>
            <div className="flex flex-col gap-4">
              {PHYSICIAN_NAV.map(({ href, title, description, icon, borderColor, iconBg, iconColor, countKey }) => (
          ...
          /* Coordinator / Admin home — upload + tier queues */
          <>
            {/* Upload zone */}
