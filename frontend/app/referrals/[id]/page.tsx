...
export default async function ReferralDetailPage({ params }: Props) {
  let referral
  try {
    referral = await getReferral(params.id)
  } catch {
    notFound()
  }
      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        ...
        <div className={`mb-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm border-l-4 ${cfg?.borderColor ?? 'border-l-slate-200'}`}>
          ...
          <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
            ...
            {referral.processed_at && (
              <span>Processed {formatRelativeTime(referral.processed_at)}</span>
            )}
            {referral.processing_time_ms && (
              <span>{(referral.processing_time_ms / 1000).toFixed(1)}s pipeline</span>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="flex flex-col gap-6">
            ...
            {referral.provider_urgency_label && (
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                <h3 className="mb-4 text-xs font-semibold uppercase tracking-wide text-slate-400">
                  Provider Urgency Label
                </h3>
                <div className="flex items-center gap-3">
                  <span className={`rounded-full px-3 py-1 text-xs font-semibold capitalize
                    ${referral.provider_urgency_label.label.toLowerCase().includes('urgent') || referral.provider_urgency_label.label.toLowerCase().includes('stat')
                      ? 'bg-amber-100 text-amber-700'
                      : 'bg-slate-100 text-slate-600'
                    }`}>
                    {referral.provider_urgency_label.label}
                  </span>
                  <span className="text-xs text-slate-400">
                    Found in: {referral.provider_urgency_label.source}
                  </span>
                </div>
              </section>
            )}
          ...
          <div className="flex flex-col gap-6">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <ActionButtons referralId={referral.id} currentStatus={referral.status} />
            </section>
          </div>
        </div>
        ...
        <div className="mt-8 flex flex-wrap gap-4 border-t border-slate-200 pt-6 text-xs text-slate-400">
          ...
          {referral.reviewed_at && (
            <span>Reviewed: {formatDateTime(referral.reviewed_at)}</span>
          )}
        </div>
      </main>
    </div>
